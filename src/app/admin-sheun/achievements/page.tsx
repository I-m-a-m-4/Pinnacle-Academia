...
import type { Admission, Subject, Academy } from '@/types';
...
export default function AchievementsPage() {
    ...
    const { triggerConfetti } = useAcademy();
    ...
    const admissionsQuery = useMemoFirebase(() => query(collection(firestore, 'admissions')), [firestore]);

    const { data: businesses, isLoading: bLoading } = useCollection<Academy>(businessesQuery);
    const { data: subjects, isLoading: pLoading } = useCollection<Subject>(subjectsQuery);
    const { data: admissions, isLoading: rLoading } = useCollection<Admission>(admissionsQuery);

    const isLoading = bLoading || pLoading || rLoading;

    const milestones: Milestone[] = useMemo(() => {
        if (!businesses || !subjects || !admissions) return [];

        const totalSessions = admissions.length;
        const totalProducts = subjects.length;
        const activeBusinesses = businesses.filter(b => b.status !== 'deleted').length;
        const totalGMV = admissions.reduce((sum, r) => sum + r.total, 0);
        ...
        return [
            ...
            // Subject Achievements
            {
                ...
                description: 'We reached 50 unique subjects listed on Zeneva.',
                target: 50,
            ...
            },
            {
                ...
                description: 'We officially host over 500 unique subjects on the platform.',
                target: 500,
                current: totalProducts,
                icon: CheckCircle,
                color: 'text-orange-500',
            },
            {
                ...
                description: '5,000 unique subjects are now available through businesses on Zeneva.',
                target: 5000,
                current: totalProducts,
                icon: Zap,
                color: 'text-pink-500',
    ...
    }, [businesses, subjects, admissions]);
