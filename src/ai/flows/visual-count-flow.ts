...
const VisualCountInputSchema = z.object({
    imageBase64: z.string().describe('The base64 encoded image of the subjects to count.'),
});
...
const prompt = ai.definePrompt({
    ...
    prompt: `You are an expert inventory assistant. Analyze the provided image and count the distinct subjects you see.
  
  For each distinct product type:
  1. Identify its name (be descriptive but concise).
  2. Count how many visible units there are.
  
  Return a structured list of items and their counts. If you cannot identify any subjects, return an empty list.`,
});
